...
extracting features across the biological rhythms that connect sleep,
cardiac function, and neural activity.

The package covers three rhythm domains: **cardiac** (QRS detection,
HRV), **respiratory** (apnoea detection, respiratory metrics), and
**neural** (spindles, slow oscillations, automatic AASM sleep staging).
Staged hypnograms pass downstream to `hypnor`; derived metrics pass to
`syncR`.
