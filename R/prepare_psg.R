...
#'
#' @param edf An `mrpheus_edf` object from [read_edf()].
...
prepare_psg <- function(edf,
  ...
  # Epoch segmentation using the first EEG channel as reference
  ...
  # Build epoch list (all channels, truncated to complete epochs)
  epochs <- lapply(seq_len(n_epochs), function(i) {
    start <- (i - 1L) * samples_per_epoch + 1L
